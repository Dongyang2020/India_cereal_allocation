# Optimization - maximize net profit
...
cs$totalProfit = cs$Profit * cs$Yield * cs$totalHA *10
cs[is.na(cs)] <- 0
...
# --> each crop in each District <= current area
...
# f. national production loss doesn't increase
df_dstr_pl = data.frame(matrix(nrow = nrow(cs), ncol = 1))
df_dstr_pl[,1] = cs$Yield*cs$totalHA*(-cs$CSensitivity)/100
one_col_dstr_pl = data.frame(x=unlist(df_dstr_pl))
...
# f. national production loss doesn't increase
cur_pl_const = sum(cs$Yield*cs$Area*(-cs$CSensitivity)/100, na.rm = T)
...
for (rg in seq(0.05,0.5,0.05)){
  ...
  Const.mat<-matrix(c(unlist(one_col_dstr), # sum of area pct in each district = 100%
                      ...
                      unlist(one_col_dstr_pl)),
                    nrow= 2*length(dstr)+1+1+3*nrow(df_vrb)+1,byrow=TRUE)
  ...
  Optimum = lp(direction="max", Objective.in,
